...
import { CLUB_EMAIL, CLUB_PHONE, MANAGER_EMAIL, ADDRESS } from '@/utils/defines/CONTACTS'
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-12">
            <div className="relative w-fit">
              ...
              <div className="absolute bottom-6 left-6 lg:bottom-12 lg:left-12 flex flex-col gap-4 text-white">
                ...
                >
                  <div className="flex items-center gap-4 group w-fit">
                    ...
                    <p className="text-sm xs:text-base mobile:text-xm font-normal group-hover:text-primary">
                      {CLUB_PHONE} - Krasimir Naumov
                    </p>
                  </div>
                </Link>
                <Link href={`mailto:${CLUB_EMAIL}`} className="w-fit">
                  <div className="flex items-center gap-4 group w-fit">
                ...
                <Link href={`mailto:${MANAGER_EMAIL}`} className="w-fit">
                  <div className="flex items-center gap-4 group w-fit">
                    <Icon icon={"ph:envelope-simple"} width={32} height={32} />
                    <p className="text-sm xs:text-base mobile:text-xm font-normal group-hover:text-primary">
                      {MANAGER_EMAIL}
                ...
                <Link href={ADDRESS} target="_blank" rel="noopener noreferrer" className="w-fit">
                  <div className="flex items-center gap-4 group w-fit">
                ...
                <div className="flex flex-col gap-8">
                  <div className="flex flex-col lg:flex-row gap-6">
                    <input
                      ...
                      required
                      className="px-6 py-3.5 border border-black/10 dark:border-white/10 rounded-md outline-primary focus:outline w-full"
                    />
                    <input
                      type="tel"
                      name="mobile"
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className="px-6 py-3.5 border border-black/10 dark:border-white/10 rounded-md outline-primary focus:outline"
                  />
                  <textarea
                    rows={8}
                    cols={50}
                  ...
                  ></textarea>
                  <button className="px-8 py-4 rounded-md bg-primary text-white text-base font-semibold w-full mobile:w-fit hover:cursor-pointer hover:bg-primary/90 duration-300">
                    {t("sendButton")}
                  </button>
                </div>
                {submitted && (
                  <h5 className="text-primary mt-4">{t("successMessage")}</h5>
                )}
              </form>
            </div>
          </div>
